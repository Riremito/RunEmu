...
bool MemoryPatch() {
	...
	// 0x00910FF2 v164.0
	ULONG_PTR uEHSvc_Loader_2 = r.Scan(L"55 8B EC 81 EC ?? ?? ?? ?? 57 C7 45 ?? 00 00 00 00 C7 45 ?? 00 00 00 00 C6 85 ?? ?? ?? ?? 00 B9 82 00 00 00 33 C0 8D BD ?? ?? ?? ?? F3 AB C7 45 ?? 00 00 00 00 E8 ?? ?? ?? ?? 90 50 E8");
	if (!uEHSvc_Loader_2) {
		// 0x00BCE382 v186.1
	...
	// 0x0090D107 v164.0
	ULONG_PTR uAutoup = r.Scan(L"56 8B F1 83 7E 14 00 74 16 68 ?? ?? ?? ?? 68 80 00 00 00 E8 ?? ?? ?? ?? 90 83 66 14 00 59 59 5E C3");
	if (!uAutoup) {
		// 0x00BC938F v186.1
